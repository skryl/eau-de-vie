...
(declare screen text-printer swing-printer init-frame clear-term reset-term)

; printing
;
(defn print-grid [grid size & {:keys [parser]}] 
  (let [printer (:printer screen)] 
    (if parser 
...
; screen
;
(defn init-no-screen [] (def screen {:printer (fn [& args] nil)} ))
(defn init-text-screen [] (def screen {:printer #(text-printer %1 %2)} ))
(defn init-swing-screen [size] 
  (let [scale 5
    ...
    (def screen  {:printer #(swing-printer %1 %2) 
                  :frame frame 
                  :panel panel 
                  :image image 
                  :scale scale})))
...
(defn swing-printer [grid size]
  (let [scale    (:scale screen)
        ssize    (* size scale)
        graphics (.createGraphics (:image screen))]
